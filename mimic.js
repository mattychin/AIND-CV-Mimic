...
function drawEmoji(canvas, img, face) {
  ...
  // <your code here>
  ctx.fillText(face.emojis.dominantEmoji, face.featurePoints[0].x-80, face.featurePoints[0].y-20);
}
...
var correctFace = false;
var timeCheck = getTimeSeconds();
var roundInterval = 5;  // each target emoji will be displayed for 5 seoconds
...
function displayEmoji() {
  targetEmoji = randomEmojiGenerator();
...
// Convert the UTC timestamp from milliseconds to seconds
...
function playGame(faces){
    ...
    // Get the time at which game is started
    var currentTime = getTimeSeconds(); 

    // If dominant emoji matches the displayed emoji, add score and move on
    if(toUnicode(faces[0].emojis.dominantEmoji) == targetEmoji){
        ...
        add_total();
        updateStats();
        timeCheck = currentTime + roundInterval;
    }

    // If time limit is exceeded, display the next target emoji and update stats
